...
<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50 px-4">
    <div class="max-w-md w-full">
      <h2 class="text-center text-2xl font-bold text-gray-800 mb-6">Sign in to Buyer dashboard</h2>
      ...
      <div class="mt-6 flex justify-between">
        <router-link to="/admin/login" class="text-indigo-600 hover:text-indigo-500"> Sign in as Admin </router-link>
...
export default {
  ...
  methods: {
    async login() {
      try {
        const response = await post('/buyer-login', {
          email: this.email,
