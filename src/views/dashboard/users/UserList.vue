<template>
  <div class="flex justify-between px-4 mt-4 sm:px-8">
    <h2 class="text-2xl text-gray-600">Buyers List</h2>
      ...
      <table class="w-full mt-2 text-gray-500">
        <thead class="border-b">
          <tr>
            ...
            <th class="text-left text-gray-600">BUYER</th>
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="buyer in filteredBuyers" :key="buyer.id">
            ...
            <td class="flex items-center py-4">
              ...
              <div class="px-4">
                <div>
                  <a href="#" class="text-gray-600 font-bolder">{{ buyer.name }}</a>
                </div>
              </div>
            </td>
            <td>{{ buyer.email }}</td>
            <td>{{ buyer.phone }}</td>
            <td>{{ buyer.created_at.split('T')[0] }}</td>
            <td class="text-right">
...
import { userGet, userDelete } from '@/plugins/http'
...
export default {
  ...
  setup() {
    ...
    const fetchBuyers = async () => {
      try {
        const response = await userGet('/admin-view-buyers')
    ...
    const deleteBuyer = async (buyerId) => {
      try {
        const confirmed = await Swal.fire({
          title: 'Are you sure?',
          text: 'This will delete the buyer from the system.',
          icon: 'warning',
        ...
        if (confirmed.isConfirmed) {
          await userDelete(`/admin-delete-buyer/${buyerId}`)
          await fetchBuyers()
          Swal.fire('Deleted!', 'The buyer has been deleted.', 'success')
        }
      } catch (error) {
        console.error('Error deleting buyer:', error)
        Swal.fire('Error', 'Failed to delete the buyer.', 'error')
      }
    }

    const filteredBuyers = computed(() => {
      if (!searchQuery.value) return buyers.value

      const regex = new RegExp(searchQuery.value, 'i')
      return buyers.value.filter(
        (buyer) => regex.test(buyer.name) || regex.test(buyer.email) || regex.test(buyer.phone)
      )
    })
