...
<template>
  ...
  <div class="grid grid-cols-1 gap-4 px-4 mt-8 sm:grid-cols-4 sm:px-8">
    ...
    <div class="flex items-center bg-white border rounded-sm overflow-hidden shadow">
      ...
      <div class="px-4 text-gray-700">
        <h3 class="text-sm tracking-wider">Delivered Orders</h3>
        <p class="text-3xl">{{ deliveredCars.toLocaleString() }}</p>
      </div>
    </div>
    ...
    <div class="flex items-center bg-white border rounded-sm overflow-hidden shadow">
      ...
      <div class="px-4 text-gray-700">
        ...
        <p class="text-lg">NGN{{ totalTransactions.toLocaleString() }}</p>
      </div>
    </div>
    ...
    <div class="flex items-center bg-white border rounded-sm overflow-hidden shadow">
      ...
      <div class="px-4 text-gray-700">
        ...
        <p class="text-3xl">{{ canceledCars.toLocaleString() }}</p>
      </div>
    </div>
  </div>

  <div class="grid grid-cols-1 gap-4 px-4 mt-8 sm:px-8">
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const response = await userGet('/buyer-dashboard')
        const data = response.data.data

        this.totalCars = data.total_orders
        this.deliveredCars = data.delivered_orders
        this.totalTransactions = data.total_amount_spent
        this.canceledCars = data.canceled_orders
      } catch (error) {
    ...
    async fetchRecentOrders() {
      try {
        const ordersResponse = await userGet('/orders')
